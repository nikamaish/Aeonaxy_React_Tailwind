            ...
            <div className="flex flex-col items-center mt-12">
                {personalData.map((data, index) => (
                    <div key={data.id} className={`flex items-center border rounded-md  mb-4 w-full p-2 md:w-1/2 text-gray-800 cursor-pointer ${selectedBox === data.id ? 'border-golden' : 'border-gray-300'}`} onClick={() => handleBoxClick(data.id)}>
                        <img src={data.image} alt="vector image" width={40} height={20} className='mr-5'/>
                        {data.title}
                    </div>
                ))}
            </div>

            <button className={`bg-black text-white p-2 px-10 mb-2 rounded-md mt-4 ${selectedBox ? '' : 'pointer-events-none opacity-50'}`} disabled={!selectedBox} > <Link to='/interest'>Continue</Link></button>
        </div>
    );
}
